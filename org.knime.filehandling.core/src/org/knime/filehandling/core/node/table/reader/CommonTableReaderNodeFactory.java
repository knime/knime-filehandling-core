 ...
 */
public abstract class CommonTableReaderNodeFactory<I, S extends Source<I>, C extends ReaderSpecificConfig<C>, T, M extends MultiTableReadConfig<C, T>, V>
     ...
     */
    protected abstract S createPathSettings(final NodeCreationConfiguration nodeCreationConfig);
    ...
    /**
     * Saves the {@link SourceSettings} and {@link MultiTableReadConfig} to {@link NodeSettingsWO} as well as validating
     * and loading them from {@link NodeSettingsRO}.
     *
     * @param <I> the item type to read from
     * @param <C> the type of {@link ReaderSpecificConfig}
     * @param <T> the type used to identify external data types
     */
    public interface ConfigAndSourceSerializer<I, S extends Source<I>, C extends ReaderSpecificConfig<C>, T, M extends MultiTableReadConfig<C, T>> {

        /**
         * Saves the given {@link SourceSettings} to the given {@link NodeSettingsWO NodeSettings}
         *
         * @param sourceSettings the source settings to save
         * @param config the multi table read config to save
         * @param settings the node settings to save to
         */
        void saveSettingsTo(S sourceSettings, M config, final NodeSettingsWO settings);

        /**
         * Validates the {@link SourceSettings} settings stored in the given {@link NodeSettingsRO NodeSettings}
         *
         * @param sourceSettings the current source settings (should not be part of the validation)
         * @param config the current multi table read config (should not be part of the validation)
         * @param settings the node settings to validate
         * @throws InvalidSettingsException if the settings are invalid
         */
        void validateSettings(S sourceSettings, M config, final NodeSettingsRO settings)
            throws InvalidSettingsException;

        /**
         * Loads the given {@link NodeSettingsRO NodeSettings} into the given {@link SourceSettings}.
         *
         * @param sourceSettings the source settings to load into
         * @param config the multi table read config to load into
         * @param settings the node settings to load from
         * @throws InvalidSettingsException
         */
        void loadValidatedSettingsFrom(S sourceSettings, M config, final NodeSettingsRO settings)
            throws InvalidSettingsException;
    }

    /**
     * @return a new {@link ConfigAndSourceSerializer} for persisting to / from the node's model settings.
     */
    ...
    public TableReaderNodeModel<I, S, C, T, M> createNodeModel(final NodeCreationConfiguration creationConfig) {
        final M config = createConfig(creationConfig);
        final S pathSettings = createPathSettings(creationConfig);
        ...
        if (portConfig.isPresent()) {
            return new TableReaderNodeModel<>(config, pathSettings, reader, serializer, portConfig.get());
        } else {
            return new TableReaderNodeModel<>(config, pathSettings, reader, serializer);
        }
    }
     ...
     * @return a new multi table reader
     */
    protected MultiTableReadFactory<I, C, T> createMultiTableReadFactory(final GenericTableReader<I, C, T, V> reader) {
        final ReadAdapterFactory<T, V> readAdapterFactory = getReadAdapterFactory();
     ...
     */
    protected abstract M createConfig(final NodeCreationConfiguration nodeCreationConfig);
